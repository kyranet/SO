...
//  idxNode inode number
//  newSize new size for the inode
//  int
...
//  mode mode
//  str output with the access mode in string format
//  void
...
//  Obtains the file attributes of a file just with the filename
...
//  Reads the content of the root directory
...
// 	typedef int( fuse_fill_dir_t)(voidbuf, const charname, const struct statstbuf, off_t off)
...
//  buf buffer where all the forder entries will be stored
//  filler Fuse funtion used to fill the buffer
//  offset offset used by filler to fill the buffer
//  fi Fuse structure associated to the directory
//  0 on success and <0 on error
...
//  path path to the root folder
//  fi Fuse structure associated to the file opened
//  0 on success and <0 on error
...
// path file path
// buf buffer where we have data to write
// size quantity of bytes to write
// offset offset over the writing
// fi Fuse structure linked to the opened file
...
//  path file path
//  mode creation mode
//  device device where the device will be created (contains both major and minor numbers)
//  0 on success and <0 on error
...
static int my_unlink(const char *path)
{
    ...
    fprintf(stderr, "--->>>my_link: removed filename %s\n", path);
    return 0;
}
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *f)
{
    ...
    int idxNode = myFileSystem.directory.files[idxDir].nodeIdx;
    if (myFileSystem.nodes[idxNode]->fileSize < offset)
        return -1;

    //bytes2Read = numero de bytes a leer (puede ser menor que size)
    if (myFileSystem.nodes[idxNode]->fileSize - offset < size)
        bytes2Read = myFileSystem.nodes[idxNode]->fileSize - offset;
    else
        bytes2Read = size;
    ...
    while (bytes2Read)
    {
        ...
        //currentBlock = bloque f´ısico en la posicion offset
        //offBlock = desplazamiento en el bloque fısico correspondiente
        currentBlock = node->blocks[offset / BLOCK_SIZE_BYTES];
        offBlock = offset % BLOCK_SIZE_BYTES;
    ...
    updateSuperBlock(&myFileSystem);
    updateBitmap(&myFileSystem);
    updateNode(&myFileSystem, f->fh, node);
